...
//
// The tip's big mass enters ONLY through constraint weighting (invMass): a
// heavy point concedes less per relax, so the light shaft wraps around it.
// That asymmetry is the whole bell-clapper feel — and the slingshot: reverse
// the anchor, the shaft bends and stores spring energy in violated bend
// constraints, then unloads through the tip.
//
...
export class VerletChain {
  ...
  rebuild() {
    ...
    for (let i = 0; i < n; i++) {
      ...
      this.invMass.push(i === 0 ? 0 : 1); // 0 = pinned: the anchor never concedes
    }
  ...
  setTipMass(m) {
    this.tipMass = m;
    this.invMass[this.points - 1] = 1 / Math.max(1e-3, m);
  }
